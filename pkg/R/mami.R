...
cat("Timing estimate unavailable under parallel computing. \n")
}
...
if(method=="MS.criterion" & (criterion!="CV" & criterion!="GCV")){
   ...
   if(inference=="+boot"){snow::clusterEvalQ(cl, "boot::boot"); snow::clusterExport(cl,"boot")}
   if(model=="cox"){snow::clusterEvalQ(cl, c("survival::coxph","survival::Surv","survival::strata","survival::cluster")); snow::clusterExport(cl, c("Surv","strata","cluster"))}
   }
...
rl <- list(mycoef, mystd, names(coefficients(full.model)) )
}, silent=FALSE)}else{
...
if(ncores>1){snow::clusterExport(cl, c("mydata1","mydata2","missing.data","imp.method","am","outcome","M","keepnames","add.parameters","model","var.remove","X","print.warnings"), envir=environment())}
...
if(ncores==1){result.boot <- try(boot(X.org,msmi.boot,B),silent=FALSE)}else{
  ...
  if(missing.data=="imputed" & imp.method=="mice"){snow::clusterEvalQ(cl, c("mice::mice","mice::complete"))}
  result.boot <- try(boot(X.org,msmi.boot,B,parallel="snow", ncpus=ncores, cl=cl),silent=FALSE)
} 
...
if(method=="LAE"){
  if(ncores>1){snow::clusterEvalQ(cl, c("glmnet::cv.glmnet","glmnet::coef.glmnet")); snow::clusterEvalQ(cl, c("quadprog::solve.QP")); snow::clusterEvalQ(cl, "boot::boot"); snow::clusterExport(cl,"boot")} 
ptm1.5 <- proc.time()
...
if(inference=="+boot"){
if(print.warnings==TRUE){cat("Now start with bootstrap estimation... \n" )}
if(ncores>1){snow::clusterExport(cl, c("missing.data","imp.method","am","outcome","M","keepnames","add.parameters","model","var.remove","X","print.warnings","outcome","cvr","ncores","kfold","save.factor"), envir=environment())}
...
if(ncores>1){snow::clusterEvalQ(cl, c("glmnet::cv.glmnet","glmnet::coef.glmnet")); 
 snow::clusterEvalQ(cl, "boot::boot"); snow::clusterExport(cl,"boot")
 if(model=="cox"){snow::clusterEvalQ(cl, c("survival::coxph","survival::Surv","survival::strata","survival::cluster")); snow::clusterExport(cl, c("Surv","strata","cluster"))}
 } 
ptm1.5 <- proc.time()           
...
if(inference=="+boot"){
if(print.warnings==TRUE){cat("Now start with bootstrap estimation... \n" )}
if(ncores>1){snow::clusterExport(cl, c("missing.data","imp.method","am","outcome","M","keepnames","add.parameters","model","var.remove","X","print.warnings","mycovariables",
...
if(model!="cox" & is.null(id)==FALSE){mymodel <- suppressWarnings(lme4::glmer(as.formula(myformula), na.action=na.fail, data = ddata, weights=X[[m]]$aweights, family=model))}  # suppress warnings since we use glmer for gaussian family
if(model=="cox"){mymodel <- survival::coxph(as.formula(myformula), data = ddata, na.action=na.fail, weights=X[[m]]$aweights)}
clusterExport(cl, list("model", "X", "m","ddata","myformula","aweights"), envir=environment())
mymo <- suppressMessages(suppressWarnings(pdredge(mymodel, evaluate=TRUE, m.lim=c((length(c(id,add.stratum))+2)*(model=="cox"),mymax+length(c(id,add.stratum))+1*(model=="cox")),rank=get(criterion),fixed=c(id,add.stratum),cluster=cl,...)))
if(model!="cox"){av_m <- suppressWarnings(ma(model.avg(mymo,subset=NA)))} else{
       if(model=="cox"){av_m <- try(suppressWarnings(ma.coxph(model.avg(mymo,subset=NA))),silent=FALSE)}}
                        sel_m <- ms(coefficients(summary(get.models(mymo,subset=1)[[1]])),names(av_m[[1]]),model)
  } else {
...
if(ncores>1){snow::clusterExport(cl, c("mydata1","mydata2","missing.data","imp.method"), envir = environment())}
...
mym.b <- suppressMessages(suppressWarnings(dredge(mymodel.b, m.lim=c((length(c(id,add.stratum))+2)*(model=="cox"),mymax+length(c(id,add.stratum))+1*(model=="cox")),rank=get(criterion),fixed=c(id,add.stratum),...)))
if(model!="cox"){av_m.b <- try(suppressWarnings(ma(model.avg(mym.b,subset=NA))),silent=FALSE)} else {
       if(model=="cox"){av_m.b <- try(suppressWarnings(ma.coxph(model.avg(mym.b,subset=NA))),silent=FALSE)}}
                        sel_mb <- try(ms(coefficients(summary(get.models(mym.b,subset=1)[[1]])),mynames,model),silent=FALSE)
  } else {
