...
mami <- function(X,method=c("criterion.average","criterion.selection","LASSO","MMA","LAE"), 
...
if(print.warnings==TRUE &(method=="criterion.selection" | method=="LASSO") & inference=="standard"){cat("Note: Bootstrap confidence intervals are preferred after model selection. Standard confidence intervals after model selection may be overoptimistic. \n")}
...
if(method=="LAE" & model.family=="coxph"){stop("Lasso averaging not possible for model.family='coxph'. However, try method='LASSO' instead.")}
if(method=="LAE" & method=="criterion.average" & model.family%in%c("gaussian","binomial","poisson")==FALSE){stop("Only the following families are allowed for LASSO averaging estimation: 'gaussian','binomial','poisson'")}
if((method=="LASSO" | method=="LAE") & is.null(id)==FALSE){stop("LASSO estimation not available for longitudinal data.")}
if(method=="LAE" & (is.null(add.transformation)==FALSE | is.null(add.interaction)==FALSE)){stop("Don't use 'add.transformation' and 'add.interaction' with method='LAE'. \n Prepare your dataset such that transformations and interactions are already contained in it.")}
if(print.warnings==TRUE & (method=="LASSO" | method=="LAE")){cat("Note: LASSO CIs based on the bootstrap SE, as reported below, are not bias corrected and not randomization valid. Interpret with caution. \n")} 
if(report.exp==TRUE & model.family=="gaussian"){warning("Exponentiated coefficients are not useful for the linear model.")}
...
cores=1
if(print.warnings==TRUE & cores>1 & ((method=="MMA" | method=="LAE" | method=="LASSO") | (method=="criterion.selection" & (criterion!="CV" & criterion!="GCV")))){cat("Note: you benefit from cores>1 only when \n 1) using 'method=criterion.average' or \n 2) 'method=criterion.selection' and 'criterion=(G)CV' \n")}
...
if(model.family=="poisson" & is.factor(X[[1]][,ycol])==TRUE){for(m in 1:M){X[[m]][,ycol] <- as.numeric(as.character(X[[m]][,ycol]))}}
if(model.family=="coxph" | model.family=="poisson"){for(m in 1:M){if(any(sign(X[[m]][,ycol])==-1)){stop("Cox/Poisson model requires non-negative outcome variables")}}}
...
if(method!="LASSO" & method!="LAE"){myvarimplist <- matrix(NA,ncol=M,nrow=length(c(usenames[-oc],param.transform,param.interaction)))}else{myvarimplist <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(ycol)+1+add.parameters-1))}
if((criterion=="BIC+" | method=="LASSO") & model.family=="coxph"){
...
if(model.family=="coxph" & class(sel_m)[1]!="coxph.null"){sel_coef <- summary(sel_m)[[7]]}
if(model.family=="coxph" & class(sel_m)[1]=="coxph.null"){sel_coef<-matrix(rep(0,dim(mycoefflist.s)[1]),ncol=1,dimnames=list(letters[1:dim(mycoefflist.s)[1]],NULL))}}
...
if(model.family=="coxph" & class(sel_m)[1]!="coxph.null"){mystd[i] <- sel_coef[j,3]}
if(model.family=="coxph" & class(sel_m)[1]=="coxph.null"){mystd[i] <- 0}}
}}}
...
if(model.family!="coxph"){sel_coef.b <- summary(sel_m.b)[[12]]} else{
   if(model.family=="coxph" & class(sel_m)[1]!="coxph.null"){sel_coef.b <- summary(sel_m.b)[[7]]}
   if(model.family=="coxph" & class(sel_m)[1]=="coxph.null"){sel_coef.b <- matrix(rep(0,dim(mycoefflist_b)[1]),ncol=1,dimnames=list(letters[1:dim(mycoefflist_b)[1]],NULL))}
   }
...
av_m <- lae(X[[m]],ycol=1,kfold=kfold, factor.variables=save.factor, calc.variance=TRUE, glm.family=model.family,pd=F,...)
mycoefflist[,m] <- av_m$coefficients[,1]
myvarlist[,m]   <- av_m$coefficients[,2]
...
for(m in 1:M)try({ 
if(print.warnings==TRUE & missing.data=="imputed"){cat("Busy with LASSO (averaging) estimation in imputed dataset no.",m, "\n")} 
av_m.b <- lae(mydata.out$imputations[[m]],ycol=1,kfold=kfold, factor.variables=NULL, calc.variance=TRUE, glm.family=model.family,pd=F,...) 
...
mydata <- mydata[indices,]
myboot <- cv.glmnet(mydata[,-1],mydata[,1],foldid=cvIndex,nfolds=kfold,family=model.family,...)
...
if(model.family=="coxph"){surv.outcome <- matrix(cbind(as.numeric(X[[m]][,1]),X[[m]][,2]),ncol=2,dimnames=list(NULL,c("time","status")))}
X[[m]] <- cbind(X[[m]][,1],model.matrix(as.formula(myformula),data=as.data.frame(X[[m]]))[,-1])
colnames(X[[m]])[1]<-keepnamesL[1]
if(model.family!="coxph"){lasso  <- cv.glmnet(X[[m]][,-1],X[[m]][,1],foldid=cvIndex,nfolds=kfold,family=model.family,...)}
if(model.family=="coxph"){lasso  <- cv.glmnet(as.matrix(X[[m]][,-1]),surv.outcome,foldid=cvIndex,nfolds=kfold,family="cox",...)}
lasso_coeff <- glmnet::coef.glmnet(lasso, s = c(lasso$lambda.min))
if(model.family!="coxph"){mylasso_boot<- boot(X[[m]],boot.lasso,max(100,B))}
if(model.family=="coxph"){mylasso_boot<- boot(cbind(surv.outcome,as.matrix(X[[m]][,-1])),boot.cox.lasso,max(100,B))}
...
mydata <- mydata[indices,]
  ...
  if(missing.data!="imputed"){mydata.out <- NULL
                              mydata.out$imputations[[1]] <- mydata} 
if(any(!(ycol %in% 1))==TRUE | is.null(var.remove)==FALSE){for(m in 1:M){mydata.out$imputations[[m]] <- try(subset(mydata.out$imputations[[m]],select=c(names(mydata.out$imputations[[m]])[ycol],names(mydata.out$imputations[[m]])[-c(ycol,var.remove)])))}}
if(model.family!="coxph"){mycoefflist_b.s <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(ycol)+1+add.parameters))}else{
mycoefflist_b.s <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(ycol)+1+add.parameters-1))}
for(m in 1:M)try({ 
if(print.warnings==TRUE & missing.data=="imputed"){cat("Busy with LASSO model selection in imputed dataset no.",m, "\n")} 
if(model.family=="coxph"){surv.outcome.b <- matrix(cbind(as.numeric(mydata.out$imputations[[m]][,1]),mydata.out$imputations[[m]][,2]),ncol=2,dimnames=list(NULL,c("time","status")))}
...
if(model.family!="coxph"){lasso.b  <- cv.glmnet(mydata.out$imputations[[m]][,-1],mydata.out$imputations[[m]][,1],foldid=cvIndex,nfolds=kfold,family=model.family,...)}
if(model.family=="coxph"){lasso.b  <- cv.glmnet(as.matrix(mydata.out$imputations[[m]][,-1]),surv.outcome.b,foldid=cvIndex,nfolds=kfold,family="cox",...)}
lasso_coeff.b <- glmnet::coef.glmnet(lasso.b, s = c(lasso.b$lambda.min))
if(model.family!="coxph"){mylasso_boot.b<- boot(mydata.out$imputations[[m]],boot.lasso,max(100,B))}
if(model.family=="coxph"){mylasso_boot.b<- boot(cbind(surv.outcome.b,as.matrix(mydata.out$imputations[[m]][,-1])),boot.cox.lasso,max(100,B))}
...
mymax <- round((dim(mycoefflist)[1]-1*((criterion=="BIC+" & model.family=="coxph")==FALSE))/(as.numeric(candidate.models==c("all"))+ 2*as.numeric(candidate.models==c("restricted"))+4*as.numeric(candidate.models==c("very restricted"))), digits=0)
...
if(criterion!="BIC+"){                           
if(model.family!="coxph" & is.null(id)){mymodel <- glm(as.formula(myformula), data = ddata, na.action=na.fail, weights=X[[m]]$user.weights, family=model.family)} 
if(model.family!="coxph" & is.null(id)==FALSE){mymodel <- suppressWarnings(glmer(as.formula(myformula), na.action=na.fail, data = ddata, weights=X[[m]]$user.weights, family=model.family))}  # suppress warnings since we use glmer for gaussian family
if(model.family=="coxph"){mymodel <- coxph(as.formula(myformula), data = ddata, na.action=na.fail, weights=X[[m]]$user.weights)}
if(cores>1){clusterExport(clust, list("model.family", "X", "m"), envir=environment())}
mymo <- suppressMessages(suppressWarnings(pdredge(mymodel, m.lim=c((length(c(id,use.stratum))+2)*(model.family=="coxph"),mymax+length(c(id,use.stratum))+1*(model.family=="coxph")),rank=get(criterion),fixed=c(id,use.stratum), cluster=clust,...)))
...
if(!(model.family!="coxph" & is.null(id)==FALSE)){sel_m <- ms(coefficients(summary(mym[[1]])),names(coefficients(mymodel)),model.family)} else{
  ...
  if(criterion=="BIC+" & model.family!="coxph"){mymodel   <- bic.glm(as.formula(myformula), data = ddata, na.action=na.fail, wt=X[[m]]$user.weights, glm.family=model.family,...)}
  if(criterion=="BIC+" & model.family=="coxph"){mymodel   <- suppressWarnings(bic.surv(as.formula(myformula), data = ddata, na.action=na.fail, wt=X[[m]]$user.weights,...))}
  av_m  <- ma.bma(mymodel)
...
if(print.warnings==TRUE & missing.data=="imputed" & (criterion=="CV" | criterion=="GCV")){cat("Busy with model selection in imputed dataset no.",m, "\n")}
if(criterion!="BIC+"){
if(model.family!="coxph" & is.null(id)){       mymodel.b <- try(glm(as.formula(myformula), na.action=na.fail, data = bddata, family=model.family))} 
if(model.family!="coxph" & is.null(id)==FALSE){mymodel.b <- suppressWarnings(glmer(as.formula(myformula), na.action=na.fail, data = bddata, family=model.family))}  # suppress warnings since we use glmer for gaussian family
if(model.family=="coxph"){                     mymodel.b <- try(coxph(as.formula(myformula), na.action=na.fail, data = bddata))}
if(cores>1){clusterExport(clust, c("model.family", "X", "m"))}
mym.b <- suppressMessages(suppressWarnings(get.models(pdredge(mymodel.b, m.lim=c(0+2*(model.family=="coxph"),mymax),rank=get(criterion),fixed=c(id,use.stratum),cluster=clust,...),subset=NA)))
...
if(criterion=="BIC+"){mynames <- make.names(names(av_m[[1]]),unique=TRUE)}
...
if(method!="criterion.selection" & method!="LASSO"){
...
if(method=="LAE" | method=="criterion.average"){
...
if(inference=="+bootstrapping" & (method!="criterion.selection" & method!="LASSO")){mycoefficients.b <- round(as.matrix(cbind(est,lower_boot,upper_boot,est_boot2,se_est_boot)),digits=5)}
if(inference=="+bootstrapping" & (method!="criterion.selection" & method!="LASSO")){
...
if(inference=="+bootstrapping" & method!="MMA"){mycoefficients.b.s <- round(as.matrix(cbind(est.s,lower_boot.s,upper_boot.s,est_boot2.s,se_est_boot.s)),digits=5)}
if(inference=="+bootstrapping"  & method!="MMA"){
...
if((method!="criterion.selection" & method!="LASSO")){mycoefficients <- mycoefficients[-1,]}
if((method!="criterion.selection" & method!="LASSO")){mycoefficients.b <- mycoefficients.b[-1,]}
if((method!="criterion.selection" & method!="LASSO")){mycoefficients.s <- mycoefficients.s[-1,]}
if((method!="criterion.selection" & method!="LASSO")){mycoefficients.b.s <- mycoefficients.b.s[-1,]}
}
if(report.exp==TRUE){
