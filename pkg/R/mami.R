...
mami <- function(X,method=c("criterion.average","criterion.selection","MMA","LASSO/LAE","WAML"), 
  criterion=c("AIC","BIC","BIC+","CV"), B=20, X.org=NULL,inference=c("standard","+bootstrapping"),
...
mymo <- suppressMessages(suppressWarnings(dredge(mymodel, m.lim=c(1,mymax),rank=get(criterion),fixed=c(id,use.stratum),...)))
if(any(is.na(mymo[,"delta"]))==FALSE | model.family=="coxph"){mym  <- suppressWarnings(get.models(mymo,subset=delta<mydelta))} else {stop("Problems in model averaging function `dredge' with calculating subset arguments. Possibly try `criterion='BIC+' '.")}
