jma <- function(y,x,method=c("JMA","MMA"),subset=c("nested","all"),factor.variables=NULL,pd=T,
                variance=c("none","boot"), bsa=100){ 

method            <- match.arg(method)
subset            <- match.arg(subset) 
variance          <- match.arg(variance) 
if(subset=="all" & ncol(x)>20){stop("Too many model combinations: \n Use either subset='nested' or reduce number of covariates")}
...
if (subset == "all"){
  ...
  if (method == "JMA") ee <- matrix(0,nrow=n,ncol=m)
  ...
  if (method == "MMA") a2 <- matrix(c(-c(sighat)*rowSums(s)),m,1)  
  if (method == "JMA") a2 <- matrix(0,nrow=m,ncol=1)
  ...
  QP <- try(solve.QP(a1,a2,a3,a4,1))
  if(class(QP)=="try-error"){QP <- try(solve.QP(make.positive.definite(a1),a2,a3,a4,1))
...
result.boot <- try(boot(cbind(y,x),jma.se.boot,bsa))
