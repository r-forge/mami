...
if(!(model.family!="coxph" & is.null(id)==FALSE)){sel_m <- ms(coefficients(summary(mym[[1]])),names(coefficients(mymodel)),model.family)} else{
  ...
  if(criterion=="BIC+" & model.family=="coxph"){mymodel   <- suppressWarnings(bic.surv(as.formula(myformula), data = as.data.frame(X[[m]]), na.action=na.fail, wt=X[[m]]$user.weights,...))}
  av_m  <- ma.bma(mymodel)
