...
mymo <- suppressMessages(suppressWarnings(MuMIn::pdredge(mymodel, evaluate=TRUE, m.lim=c((length(c(id,add.stratum))+2)*(model=="cox"),mymax+length(c(id,add.stratum))+1*(model=="cox")),rank=get(criterion),fixed=c(id,add.stratum),cluster=NA,...)))
