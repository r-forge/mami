...
if(model=="quasipoisson"){stop("Quasi-likelihood methods are not supported yet.")}
...
if(print.warnings==TRUE &(method=="MS.criterion" | method=="LASSO") & inference=="standard"){cat("Note: Bootstrap confidence intervals are preferred after model selection. Standard confidence intervals after model selection may be overoptimistic. \n")}
...
if(method=="MS.criterion" & criterion=="BIC" & model=="cox"){stop("BIC not allowed for Cox model. `N' is not clearly defined.")}
if(method=="LAE" & model=="cox"){stop("Lasso averaging not possible for model='cox'. However, try method='LASSO' instead.")}
if(method=="LAE" & method=="MA.criterion" & model%in%c("gaussian","binomial","poisson")==FALSE){stop("Only the following families are allowed for LASSO averaging estimation: 'gaussian','binomial','poisson'")}
...
if(report.exp==TRUE & model=="gaussian"){warning("Exponentiated coefficients are not useful for the linear model.")}
...
if(print.warnings==TRUE & criterion=="BIC+" & method=="MS.criterion"){stop("You can use `criterion=BIC' for model selection. `BIC+' is needed for model averaging, particularly when analysing a big dataset.")}
...
cores=1
if(print.warnings==TRUE & cores>1 & ((method=="MMA" | method=="LAE" | method=="LASSO") | (method=="MS.criterion" & (criterion!="CV" & criterion!="GCV")))){cat("Note: you benefit from cores>1 only when \n 1) using 'method=MA.criterion' or \n 2) 'method=MS.criterion' and 'criterion=(G)CV' \n")}
...
sit <- apply(as.matrix(sapply(exclude, function(exclude) grepl(exclude, add.transformation))),1,sum)>0
add.transformation <- add.transformation[!sit]
if(print.warnings==T){if(any(sit)){cat(paste("Due to screening the list of suggested transformations has been adjusted. \n"))}}
}
if(is.null(add.interaction)==F){
sii <- apply(as.matrix(sapply(exclude, function(exclude) grepl(exclude, add.interaction))),1,sum)>0
add.interaction <- add.interaction[!sii]
if(print.warnings==T){if(any(sii)){cat(paste("Due to screening the list of suggested interactions has been adjusted. \n"))}}
}
if(is.null(add.factor)==F){
sif <- apply(as.matrix(sapply(exclude, function(exclude) grepl(exclude, add.factor))),1,sum)>0
add.factor <- add.factor[!sif]
if(print.warnings==T){if(any(sif)){cat(paste("Due to screening the list of suggested factors has been adjusted. \n"))}}
}
}
...
mycoefflist <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(outcome)+1+add.parameters))
myvarlist   <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(outcome)+1+add.parameters))
mycoefflist.s <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(outcome)+1+add.parameters))
myvarlist.s <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(outcome)+1+add.parameters))
...
mycoefflist <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(outcome)+1+add.parameters-1))
myvarlist   <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(outcome)+1+add.parameters-1))
mycoefflist.s <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(outcome)+1+add.parameters-1))
myvarlist.s <- matrix(NA,ncol=M,nrow=(length(keepnames)-length(outcome)+1+add.parameters-1))
...
if(model!="cox"){av_m <- suppressWarnings(ma(model.avg(mym)))} else{
       if(model=="cox"){av_m <- try(suppressWarnings(ma.coxph(model.avg(mym))))}}
if(!(model!="cox" & is.null(id)==FALSE)){sel_m <- ms(coefficients(summary(mym[[1]])),names(coefficients(mymodel)),model)} else{
                                                  sel_m <- ms(coefficients(summary(mym[[1]])),rownames(summary(mymodel)[[10]]),model)             }
  } else {
...
inbetweentime <- round((((ptm2-ptm1.5)/60))[1],digits=2)
if(print.time==TRUE & inference=="+boot"){cat(paste("The analysis will run for about another", inbetweentime*(B+1), "minute(s) \n"))}
if(!(model!="cox" & is.null(id)==FALSE)){mynames <- names(mymodel$coefficients)} else{
...
mym.b <- suppressMessages(suppressWarnings(get.models(pdredge(mymodel.b, m.lim=c(0+2*(model=="cox"),mymax),rank=get(criterion),fixed=c(id,add.stratum),cluster=clust,...),subset=NA)))
if(model!="cox"){av_m.b <- try(suppressWarnings(ma(model.avg(mym.b))))} else {
       if(model=="cox"){av_m.b <- try(suppressWarnings(ma.coxph(model.avg(mym.b))))}}
if(!(model!="cox" & is.null(id)==FALSE)){sel_mb <- try(ms(coefficients(summary(mym.b[[1]])),mynames,model))} else{
...
if(inference=="+boot" & (method!="MS.criterion" & method!="LASSO")){mycoefficients.b <- round(as.matrix(cbind(est,lower_boot,upper_boot,est_boot2,se_est_boot)),digits=5)}
...
if(inference=="+boot" & method!="MMA"){mycoefficients.b.s <- round(as.matrix(cbind(est.s,lower_boot.s,upper_boot.s,est_boot2.s,se_est_boot.s)),digits=5)}
...
res= list(coefficients.ma=mycoefficients,
          ...
          setup = list(missing.data,model,outcome,id,method,criterion,kfold,cvr,
                       ...
                       add.transformation,add.stratum,screen,exclude)
)
