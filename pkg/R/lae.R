...
shrink_sel  <- var_shrink_sel <- shrink_ave <- var_shrink_ave <- sae_weights <- sel_weights <- complexity_parameter <- variable_importance <- NULL       
...
lasso <- glmnet::cv.glmnet(X[,-ycol],X[,ycol],foldid=cvIndex,nfolds=kfold,family=glm.family, standardize=standardize, ...)
...
# Shrinkage averaging estimation = OCV_k 
        ...
        myavlasso <-  glmnet::cv.glmnet(myavdata[,-ycol2],myavdata[,ycol2],lambda=newlambda,foldid=cvIndex4,family=glm.family, standardize=standardize, ...)
        ...
        sae_weights <- NULL
        tt<-1
        sae_weights <- suppressWarnings(try(SAE(X,lasso$lambda,kfold),silent=TRUE))
        ...
        sae_weights <- try(SAE(X,lasso$lambda,kfold,reshuffle=TRUE),silent=TRUE)
        print(tt)
...
myboot <- glmnet::cv.glmnet(mydata[,-1],mydata[,1],nfolds=kfold,lambda=lasso$lambda,family=glm.family, standardize=standardize, ...)
