...
mami <- function(X,method=c("criterion.average","criterion.selection","MMA","LASSO/LAE","WAML"), criterion=c("AIC","BIC","CV"), B=20, X.org=NULL,
                  inference=c("standard","+bootstrapping"), missing.data=c("imputed","none","CC"), var.remove=NULL, user.weights=NULL,
                  candidate.models=c("all","restricted","very restricted"), model.family=c("gaussian","binomial","poisson","coxph"),
                  add.factor=NULL, add.interaction=NULL, add.transformation=NULL, ycol=1, CI=0.95, kfold=5, id=NULL, use.stratum=NULL,
...
if(method=="WAML"){stop("Weighted Average Least Squares and Maximum Likelihood to be added soon.")}
if(model.family=="coxph" & criterion=="CV"){stop("Cross Validation not allowed in the Cox PH model.")}
if(is.null(id)==FALSE  & criterion=="CV"){stop("Cross Validation not allowed for longitudinal data.")}
if(criterion=="CV" & method=="criterion.selection"){stop("Choose `method=criterion.average' to obtain the model selection results with cross validation.")}
...
if((candidate.models=="restricted" | candidate.models=="very restricted") & print.warnings==TRUE){cat(paste("Note: Due to your specified restriction the maximum amount of variables allowed in the candidate models is ",mymax,".\n",sep=""))} 
...
if(sum(mymodel$prior.weights!=1)!=0){warning("Cross validation should not be used with weights")}
cvIndex <- rep(1:kf,trunc(length(mymodel$y)/kf)+1)[1:length(mymodel$y)]
    cvIndex <- sample(cvIndex)
    cv <-0
          for (j in 1:kf){
                mydata   <<- as.data.frame(mymodel$data[cvIndex!=j,])
                mymodelcv  <- glm(mymodel$formula, data=mydata, weights=mydata$prior.weights, family=mymodel$family)
...
mymo <- suppressMessages(suppressWarnings(dredge(mymodel, m.max=mymax, m.min=1,rank=get(criterion),fixed=c(id,use.stratum),...)))
if(any(is.na(mymo[,"delta"]))==FALSE){mym  <- suppressWarnings(get.models(mymo,subset=delta<mydelta))} else {stop("Problems in model averaging function `dredge' with calculating subset arguments.")}
...
mym.b <- suppressMessages(suppressWarnings(get.models(dredge(mymodel.b, m.max=mymax, m.min=1,rank=get(criterion),fixed=c(id,use.stratum),...),subset=delta<mydelta)))
...
if(report.exp==TRUE){
if(is.null(mycoefficients)==FALSE){mycoefficients <- round(exp(mycoefficients[,-2]),digits=4)}
if(is.null(mycoefficients.b)==FALSE){mycoefficients.b <- round(exp(mycoefficients.b[,-3]),digits=4)}
if(is.null(mycoefficients.s)==FALSE){mycoefficients.s <- round(exp(mycoefficients.s[,-2]),digits=4)}
if(is.null(mycoefficients.b.s)==FALSE){mycoefficients.b.s <- round(exp(mycoefficients.b.s[,-3]),digits=4)}
}
...
res= list(coefficients=mycoefficients,
          coefficients.boot = mycoefficients.b,
